...
X, Y = create_placeholders(240, 2404, 3, 4)
print ("X = " + str(X))
print ("Y = " + str(Y))
...
with tf.Session() as sess_test:
    ...
    def forward_propagation(X, parameters):
        ...
        Z3 = tf.contrib.layers.fully_connected(F, 4, activation_fn=None)
        ### END CODE HERE ###
    ...
    with tf.Session() as sess:
        np.random.seed(1)
        X, Y = create_placeholders(240, 240, 3, 4)
        parameters = initialize_parameters()
        Z3 = forward_propagation(X, parameters)
        init = tf.global_variables_initializer()
        sess.run(init)
        a = sess.run(Z3, {X: np.random.randn(2, 240, 240, 3), Y: np.random.randn(2, 4)})
        print("Z3 = \n" + str(a))
    ...
    with tf.Session() as sess:
        ...
        init = tf.global_variables_initializer()
        sess.run(init)
        a = sess.run(cost, {X: np.random.randn(4, 240, 240, 3), Y: np.random.randn(4, 4)})
        print("cost = " + str(a))
